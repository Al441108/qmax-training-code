...
def load_annotations(path: Path) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    skipped_missing_geometry: List[Dict[str, Any]] = []
    skipped_degenerate: List[Dict[str, Any]] = []
    raw_count = 0
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for raw in reader:
            ...
            if any(value == "" for value in raw_geometry.values()):
                skipped_missing_geometry.append(
                    {
                        "file": str(raw["file"]),
                        "slice": str(raw.get("slice", "")),
                        "label": str(raw["label"]).strip(),
                        **raw_geometry,
                        "reason": "missing_bounding_box_geometry",
                    }
                )
                continue
            try:
                ...
                raise RuntimeError(
                    "Non-numeric non-empty fastMRI+ bounding-box geometry: "
                    f"file={raw['file']} slice={raw['slice']} "
                    f"geometry={raw_geometry}"
                ) from error
            if not all(math.isfinite(value) for value in geometry.values()):
                raise RuntimeError(
                    "Non-finite fastMRI+ bounding-box geometry: "
                    f"file={raw['file']} slice={raw['slice']} geometry={geometry}"
                )
            if geometry["width"] <= 0.0 or geometry["height"] <= 0.0:
                skipped_degenerate.append(
                    {
                        "file": str(raw["file"]),
                        "slice": int(float(raw["slice"])),
                        "label": str(raw["label"]).strip(),
                        **geometry,
    ...
    annotation_audit = {
        "num_csv_rows": int(raw_count),
        "num_usable_annotations": len(rows),
        "num_skipped_missing_geometry": len(skipped_missing_geometry),
        "skipped_missing_geometry": skipped_missing_geometry,
        "num_skipped_degenerate_annotations": len(skipped_degenerate),
        "skipped_degenerate_annotations": skipped_degenerate,
        "degenerate_policy": (
            "Rows with blank geometry are recorded as non-box/incomplete and "
            "excluded. Rows with width <= 0 or height <= 0 are excluded because "
            "they contain no ROI pixels. Non-empty non-numeric or non-finite "
            "geometry remains a hard error."
        ),
    }
    return rows, annotation_audit
...
def run_preflight(args: argparse.Namespace) -> None:
    ...
    annotations, annotation_audit = load_annotations(Path(args.annotations_csv))
