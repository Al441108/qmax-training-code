...
def find_single_ckpt(R):
    ...
    # Prefer paths containing pdfs and R{R}
    candidates = [
        p for p in files
        if "pdfs" in str(p).lower()
    ...
    # Prefer model_best.pt if available
    candidates = sorted(
        candidates,
        key=lambda p: (
    ...
    print(f"Using single checkpoint for R={R}: {candidates[0]}")
...
for R in [4, 6, 8]:
    ...
    cmd = [
        ...
        "--output_dir", "outputs/figures/slice34_R4_R6_R8_PDFS"
    ]

    subprocess.run(cmd, check=True)

print("\nDone. Figures saved to outputs/figures/slice34_R4_R6_R8_PDFS")
