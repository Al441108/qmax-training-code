...
from src.m2_prnf_rpcrf_varnet import (  # noqa: E402
    FUSION_DESIGNS,
    M2PRNFRPCRFVarNet,
    PRNF_VARIANTS,
)
...
def evaluate_clean(model, loader, device, max_batches=None) -> Dict[str, Any]:
    ...
    direct_values, residual_values = [], []
    residual_max_values, residual_ratio_values = [], []
    raw_aux_values, cosine_values = [], []
    for batch_index, batch in enumerate(loader, start=1):
        if max_batches is not None and batch_index > max_batches:
            break
        kspace, mask, pd, target, _ = prepare_batch(batch, device)
        # Causal validation intervention: compare the trained hybrid against
        # the *same checkpoint* with only its complementary residual disabled.
        # This avoids attributing an improvement to unrelated parameter or
        # optimisation differences between independently trained models.
        with residual_scale_override(model, 0.0):
            reference = model(
                kspace, mask, pd, torch.ones(pd.shape[0], device=device)
            )
            reference = center_crop(
                reference, target.shape[-2], target.shape[-1]
            )
            reference_losses = l1_per_sample(reference, target)
        ...
        direct_values.extend(aux["direct_to_target_rms"].mean((1, 2)).cpu().tolist())
        residual_values.extend(aux["residual_to_target_rms"].mean((1, 2)).cpu().tolist())
        residual_max_values.extend(
            aux["residual_to_target_rms"].amax((1, 2)).cpu().tolist()
        )
        residual_ratio_values.extend(
            aux["residual_to_direct_rms_ratio"].mean((1, 2)).cpu().tolist()
        )
        raw_aux_values.extend(
            aux["raw_auxiliary_to_target_rms"].mean((1, 2)).cpu().tolist()
        )
        cosine_values.extend(
            aux["target_auxiliary_cosine"].mean((1, 2)).cpu().tolist()
        )
            ...
            slice_rows.append(
                {
                    ...
                    "residual_off_l1": float(reference_losses[i].item()),
                }
            )
    patient_values: Dict[str, Dict[str, List[float]]] = defaultdict(
        lambda: {"l1": [], "residual_off_l1": []}
    )
    for row in slice_rows:
        patient_values[row["patient_id"]]["l1"].append(row["l1"])
        patient_values[row["patient_id"]]["residual_off_l1"].append(
            row["residual_off_l1"]
        )
    patient_rows = [
        {
            "patient_id": key,
            "num_slices": len(values["l1"]),
            "l1": safe_mean(values["l1"]),
            "residual_off_l1": safe_mean(values["residual_off_l1"]),
        }
    ...
    return {
        ...
        "residual_off_patient_l1": safe_mean(
            row["residual_off_l1"] for row in patient_rows
        ),
        "hybrid_minus_residual_off_patient_l1": safe_mean(
            row["l1"] - row["residual_off_l1"] for row in patient_rows
        ),
        "patients_hybrid_better_than_residual_off": sum(
            row["l1"] < row["residual_off_l1"] for row in patient_rows
        ),
        "clean_q": safe_mean(q_values),
        "clean_need": safe_mean(need_values),
        "clean_gated_rms": safe_mean(rms_values),
        "clean_direct_rms": safe_mean(direct_values),
        "clean_residual_rms": safe_mean(residual_values),
        "clean_residual_rms_max": max(residual_max_values, default=0.0),
        "clean_residual_direct_ratio": safe_mean(residual_ratio_values),
        "clean_raw_auxiliary_rms": safe_mean(raw_aux_values),
        "clean_target_auxiliary_cosine": safe_mean(cosine_values),
...
IMMUTABLE_RESUME_KEYS = (
    "variant", "fusion_design", "need_scope", "residual_scale",
    "residual_reliability_power", "residual_levels", "run_stage",
    "metadata_csv", "metadata_identity_csv", "metadata_sha256",
    "metadata_runtime_sha256", "condition_manifest",
    "condition_manifest_sha256",
    "acceleration", "pd_aux_acceleration",
    ...
    "lambda_rel", "lambda_rank", "aux_loss_ramp_epochs",
    "lambda_residual_gain", "residual_gain_margin_relative",
    "lambda_correction", "lambda_overshoot", "overshoot_kappa",
    "correction_weight_clip", "residual_gain_ramp_epochs", "seed",
...
@contextmanager
def residual_scale_override(model: M2PRNFRPCRFVarNet, value: float):
    """Temporarily set every complementary residual scale without changing state.

    The clean direct-only reference is evaluated sequentially under no_grad, so
    the full hybrid graph remains the only graph resident during backward.
    """
    complements = [
        controller.complement
        for controller in model.controllers
        if controller.complement is not None
    ]
    originals = [module.residual_scale.detach().clone() for module in complements]
    with torch.no_grad():
        for module in complements:
            module.residual_scale.fill_(float(value))
    try:
        yield
    finally:
        with torch.no_grad():
...
def residual_trainable_parameters(model: M2PRNFRPCRFVarNet):
    """Return only the complementary-generator and local-gate parameters."""
    selected = [
        parameter
        for name, parameter in model.named_parameters()
        if ".complement." in name and parameter.requires_grad
    ]
    if not selected:
        raise RuntimeError("No trainable complementary-residual parameters found")
    return selected


def corrective_residual_losses(
    prediction: torch.Tensor,
    residual_off: torch.Tensor,
    target: torch.Tensor,
    weight_clip: float,
    overshoot_kappa: float,
):
    """Supervise the residual-induced image correction, not a scalar proxy.

    Both the residual-off image and its remaining error are stop-gradient
    targets.  A per-sample continuous weight focuses learning on unresolved
    errors while clipping prevents isolated pixels from dominating.
    """
    scale = target.amax(dim=(-2, -1), keepdim=True).clamp_min(1e-8)
    off = (residual_off / scale).detach()
    target_n = target / scale
    correction = prediction / scale - off
    remaining_error = (target_n - off).detach()
    weights = (
        remaining_error.abs()
        / remaining_error.abs().mean(dim=(-2, -1), keepdim=True).clamp_min(1e-8)
    ).clamp(max=float(weight_clip)).detach()
    correction_loss = F.smooth_l1_loss(
        weights * correction, weights * remaining_error
    )
    overshoot = F.relu(
        correction.abs() - float(overshoot_kappa) * remaining_error.abs()
    )
    overshoot_loss = overshoot.mean()
    direction_cosine = F.cosine_similarity(
        correction.detach().flatten(1), remaining_error.flatten(1), dim=1
    ).mean()
    overshoot_rate = (
        correction.detach().abs()
        > float(overshoot_kappa) * remaining_error.abs()
    ).float().mean()
    return correction_loss, overshoot_loss, direction_cosine, overshoot_rate


def add_selected_gradients(loss, parameters, scale: float = 1.0) -> float:
    """Route an auxiliary objective only to explicitly selected parameters."""
...
def validate_resume_config(previous: Dict[str, Any], current: Dict[str, Any]) -> None:
    """Compare semantic JSON values, not Python container implementation types.

    Checkpoint configs retain tuples while the mirrored config.json necessarily
    restores them as lists.  Canonical JSON normalisation prevents a false
    resume rejection without relaxing any value-level protocol check.
    """
    ...
    for key in IMMUTABLE_RESUME_KEYS:
        if canonical(previous.get(key)) != canonical(current.get(key)):
            mismatches[key] = {"checkpoint": previous.get(key), "current": current.get(key)}
    if mismatches:
        raise RuntimeError(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Formal 50-epoch H/4-only RPCRF training with "
            "direction-corrective supervision on Isambard."
        )
    )
    parser.add_argument("--metadata_csv", required=True)
    parser.add_argument(
        "--metadata_identity_csv",
        required=True,
        help="Frozen Isambard metadata identity; runtime CSV may only relocate paths.",
    )
    parser.add_argument("--output_dir", required=True)
    parser.add_argument("--full_clean_manifest", required=True)
    parser.add_argument("--robustness_manifest", required=True)
    parser.add_argument("--variant", required=True, choices=sorted(PRNF_VARIANTS))
    parser.add_argument("--fusion_design", required=True, choices=sorted(FUSION_DESIGNS))
    parser.add_argument(
        "--run_stage", required=True, choices=["rpcrf_h4_formal50"]
    )
    parser.add_argument(
        "--need_scope", choices=["residual", "all_auxiliary"], default="residual",
        help="Compatibility field; the registered formal model uses prnf_no_need.",
    )
    parser.add_argument("--residual_scale", type=float, default=0.1)
    parser.add_argument("--residual_reliability_power", type=float, default=1.5)
    ...
    parser.add_argument(
        ...
        help=(
            "Memory-safe Isambard anatomical microbatch; paired clean/corrupt "
            "forward batch is 2x, therefore four images."
        ),
    )
    parser.add_argument(
        "--grad_accum_steps", type=int, default=2,
        help="Accumulate two microbatches to preserve effective anatomical batch 4.",
    )
    ...
    parser.add_argument("--lambda_residual_gain", type=float, default=0.10)
    parser.add_argument(
        "--residual_gain_margin_relative", type=float, default=0.002,
        help="Required per-sample hybrid improvement over residual-off clean L1.",
    )
    parser.add_argument("--residual_gain_ramp_epochs", type=int, default=5)
    parser.add_argument("--lambda_correction", type=float, default=0.05)
    parser.add_argument("--lambda_overshoot", type=float, default=0.01)
    parser.add_argument("--overshoot_kappa", type=float, default=1.25)
    parser.add_argument("--correction_weight_clip", type=float, default=4.0)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--resume", default=None)
    args = parser.parse_args()

    if args.variant != "prnf_no_need":
        raise ValueError("H/4 RPCRF formal model must use prnf_no_need")
    if args.fusion_design != "hybrid_direct_residual":
        raise ValueError("H/4 RPCRF formal model requires hybrid_direct_residual")
    if args.need_scope != "residual":
        raise ValueError("H/4 RPCRF formal model requires need_scope=residual")
    if args.epochs != 50:
        raise ValueError("RPCRF formal Isambard run requires exactly 50 epochs")
    if args.batch_size != 2 or args.grad_accum_steps != 2:
        raise ValueError(
            "Isambard memory-safe formal protocol requires batch_size=2, "
            "grad_accum_steps=2 (effective anatomical batch 4)"
        )
    ...
    if args.learning_rate <= 0 or min(
        args.lambda_rel, args.lambda_rank, args.lambda_residual_gain,
        args.lambda_correction, args.lambda_overshoot
    ) < 0:
        raise ValueError("Invalid learning rate or auxiliary loss weight")
    if not 0.0 <= args.residual_gain_margin_relative < 0.05:
        raise ValueError("residual_gain_margin_relative must lie in [0,0.05)")
    if args.residual_gain_ramp_epochs < 1:
        raise ValueError("residual_gain_ramp_epochs must be positive")
    if args.residual_reliability_power < 1.0:
        raise ValueError("residual_reliability_power must be >= 1")
    if args.overshoot_kappa <= 0 or args.correction_weight_clip <= 0:
        raise ValueError("overshoot_kappa and correction_weight_clip must be positive")
    ...
    args.metadata_identity_csv = str(Path(args.metadata_identity_csv).resolve())
    if not Path(args.metadata_identity_csv).is_file():
        raise FileNotFoundError(args.metadata_identity_csv)
    for name in (
        "full_clean_manifest", "robustness_manifest", "condition_manifest"
    ):
    ...
    set_seed(args.seed)
    model = M2PRNFRPCRFVarNet(
        model_variant=args.variant,
        fusion_design=args.fusion_design,
        need_scope=args.need_scope,
        residual_scale=args.residual_scale,
        residual_reliability_power=args.residual_reliability_power,
        residual_levels=(2,),
    ...
    config.update(
        {
            ...
            "protocol_version": (
                "M2-PRNF-R8-v1.7.4-RPCRF-H4-formal50-Isambard-b2a2"
            ),
            "fusion_hypothesis": (
                "make the bounded complementary residual approximate the "
                "residual-off reconstruction error while retaining the stable "
                "global-direct path and pair-aware reliability protection"
            ),
            "training_objective": (
                "h4_direction_corrective_residual_only_routing"
            ),
            "gain_reference": (
                "same model, same clean batch, actual reliability, residual_scale=0, "
                "sequential no_grad"
            ),
            "gain_reference_stop_gradient": True,
            "gain_supervision_scope": "clean paired PD only",
            "correction_gradient_scope": (
                "controllers.H4.complement.* only; backbone, direct path and "
                "reliability receive reconstruction/reliability objectives only"
            ),
            "direct_reliability": "q",
            "residual_reliability": "q ** residual_reliability_power",
            "scale_selection": (
                "H/4 only, frozen before sixth-arm training from the completed "
                "same-checkpoint Step-2 scale intervention"
            ),
            "residual_levels": ["H/4"],
            "residual_level_indices": [2],
            "microbatch_adaptation": {
                "reason": (
                    "Memory-safe Isambard adaptation after batch4/accum1 exceeded "
                    "the 95-GiB GPU capacity during the paired clean/corrupt forward"
                ),
                "anatomical_microbatch": args.batch_size,
                "paired_forward_batch": 2 * args.batch_size,
                "gradient_accumulation": args.grad_accum_steps,
                "effective_anatomical_batch": (
                    args.batch_size * args.grad_accum_steps
                ),
                "reference_effective_anatomical_batch": 4,
            },
            "shared_v13_controls_reusable": True,
            "execution_status": (
                "speculative parallel formal run; scientific eligibility is "
                "conditional on the locked 15-epoch pilot gate"
            ),
            "optimizer": {
            ...
            "condition_manifest_sha256": sha256_file(Path(args.condition_manifest)),
            # Preserve the original metadata identity for comparison with
            # Isambard-trained controls; runtime paths are audited separately.
            "metadata_sha256": sha256_file(Path(args.metadata_identity_csv)),
            "metadata_runtime_sha256": sha256_file(Path(args.metadata_csv)),
    ...
    fieldnames = [
        ...
        "train_residual_gain_loss", "train_residual_gain_ramp",
        "train_correction_loss", "train_overshoot_loss",
        "train_correction_direction_cosine", "train_overshoot_rate",
        "train_routed_residual_gradient_norm",
        "train_direct_reference_l1", "train_hybrid_minus_direct_l1",
        "train_residual_gain_violation_rate",
        ...
        "train_direct_rms_clean", "train_direct_rms_corrupt",
        "train_residual_rms_clean", "train_residual_rms_corrupt",
        "train_residual_rms_max_clean", "train_residual_rms_max_corrupt",
        "train_residual_direct_ratio_clean",
        "train_residual_direct_ratio_corrupt",
        "train_raw_auxiliary_rms_clean", "train_raw_auxiliary_rms_corrupt",
        "train_target_auxiliary_cosine_clean",
        "train_target_auxiliary_cosine_corrupt",
        "val_patient_l1", "val_slice_l1", "val_clean_q", "val_clean_need",
        "val_residual_off_patient_l1",
        "val_hybrid_minus_residual_off_patient_l1",
        "val_patients_hybrid_better_than_residual_off",
        "val_clean_gated_rms", "val_clean_direct_rms", "val_clean_residual_rms",
        "val_clean_residual_rms_max", "val_clean_residual_direct_ratio",
        "val_clean_raw_auxiliary_rms", "val_clean_target_auxiliary_cosine",
        "gradient_norm_mean", "epoch_seconds",
        "peak_gpu_memory_gb",
    ]
    if start_epoch == 1:
    ...
    print(json.dumps({
        ...
        "fusion_design": args.fusion_design, "need_scope": args.need_scope,
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch_index, batch in enumerate(train_loader, start=1):
            ...
            # Sequential clean residual-off reference.  It uses the same model,
            # actual learned reliability and identical input, but no residual
            # contribution.  no_grad prevents a second graph from exceeding the
            # 95-GiB allocation visible on the target GH200.
            with torch.no_grad(), residual_scale_override(model, 0.0):
                direct_reference = model(
                    kspace, mask, pd,
                    torch.ones(base, device=device),
                )
                direct_reference = center_crop(
                    direct_reference, target.shape[-2], target.shape[-1]
                )
                direct_reference_l1_per_sample = l1_per_sample(
            ...
            ) * direct_reference_l1_per_sample
            residual_gain_per_sample = F.relu(
                clean_l1_per_sample - required_clean_l1
            )
            residual_gain_loss = residual_gain_per_sample.mean()
            residual_gain_violation_rate = (
                clean_l1_per_sample > required_clean_l1
            ).float().mean()
            hybrid_minus_direct_l1 = (
            ...
            gain_ramp = min(
                1.0, epoch / max(1, args.residual_gain_ramp_epochs)
            )
            main_loss = recon_loss + ramp * (
            ...
            for diagnostic_name in (
                "direct_to_target_rms", "residual_to_target_rms",
                "gated_aux_to_target_rms", "raw_residual_to_target_rms",
            ):
                if not bool(torch.isfinite(aux[diagnostic_name]).all()):
                    raise RuntimeError(
                        f"Non-finite {diagnostic_name} at epoch {epoch}, "
                        f"batch {batch_index}"
                    )
            maximum_residual_ratio = float(
                aux["residual_to_target_rms"].detach().max().item()
            )
            if maximum_residual_ratio > 5.0:
                raise RuntimeError(
                    "Emergency residual-dominance stop: relative RMS "
                    f"{maximum_residual_ratio:.3f} exceeds 5.0"
                )
            # First backward: the ordinary reconstruction/reliability objective
            # updates the whole network.  The graph is released before the
            # correction pass. Divide by accumulation so batch2/accum2 has the
            # same gradient scale as the registered effective batch of four.
            (main_loss / args.grad_accum_steps).backward()

            # Second clean forward: correction-specific objectives are routed
            # only to complementary residual parameters via autograd.grad.
            correction_prediction = model(
                kspace, mask, pd, torch.ones(base, device=device)
            )
            correction_prediction = center_crop(
                correction_prediction, target.shape[-2], target.shape[-1]
            )
            correction_loss, overshoot_loss, direction_cosine, overshoot_rate = (
                corrective_residual_losses(
                    correction_prediction, direct_reference, target,
                    args.correction_weight_clip, args.overshoot_kappa,
                )
            )
            correction_l1_per_sample = l1_per_sample(
                correction_prediction, target
            )
            routed_gain_loss = F.relu(
                correction_l1_per_sample - required_clean_l1
            ).mean()
            routed_objective = gain_ramp * (
                args.lambda_correction * correction_loss
                + args.lambda_overshoot * overshoot_loss
                + args.lambda_residual_gain * routed_gain_loss
            )
            if not torch.isfinite(routed_objective):
                raise RuntimeError("Non-finite routed residual objective")
            routed_gradient_norm = add_selected_gradients(
                routed_objective,
                residual_trainable_parameters(model),
                scale=1.0 / args.grad_accum_steps,
            )
            total_loss = main_loss.detach() + routed_objective.detach()
            del correction_prediction, direct_reference
            accumulation += 1
            ...
            for key, value in {
                ...
                "gain": routed_gain_loss,
                "correction": correction_loss,
                "overshoot": overshoot_loss,
                "direction_cosine": direction_cosine,
                "overshoot_rate": overshoot_rate,
                "routed_gradient": torch.as_tensor(
                    routed_gradient_norm, device=device
                ),
                "direct_reference": direct_reference_l1_per_sample.mean(),
                "hybrid_minus_direct": hybrid_minus_direct_l1,
                "gain_violation": residual_gain_violation_rate,
            }.items():
            ...
            for key, tensor in {
                ...
                "direct_rms_clean": aux["direct_to_target_rms"][:base],
                "direct_rms_corrupt": aux["direct_to_target_rms"][base:],
                "residual_rms_clean": aux["residual_to_target_rms"][:base],
                "residual_rms_corrupt": aux["residual_to_target_rms"][base:],
                "residual_rms_max_clean": aux["residual_to_target_rms"][:base].amax((1, 2)),
                "residual_rms_max_corrupt": aux["residual_to_target_rms"][base:].amax((1, 2)),
                "residual_ratio_clean": aux["residual_to_direct_rms_ratio"][:base],
                "residual_ratio_corrupt": aux["residual_to_direct_rms_ratio"][base:],
                "raw_aux_rms_clean": aux["raw_auxiliary_to_target_rms"][:base],
                "raw_aux_rms_corrupt": aux["raw_auxiliary_to_target_rms"][base:],
                "feature_cosine_clean": aux["target_auxiliary_cosine"][:base],
                "feature_cosine_corrupt": aux["target_auxiliary_cosine"][base:],
            }.items():
                reducer = tensor.detach().max() if "rms_max" in key else tensor.detach().mean()
                stats[key].append(float(reducer.item()))
            if batch_index == 1 or batch_index % 25 == 0:
                print(
                    ...
                    f"direct_ref={direct_reference_l1_per_sample.mean().item():.6f} "
                    f"gain={routed_gain_loss.item():.6f} "
                    f"corr={correction_loss.item():.6f} "
                    f"over={overshoot_loss.item():.6f} "
        ...
        row = {
            ...
            "train_residual_gain_loss": safe_mean(stats["gain"]),
            "train_correction_loss": safe_mean(stats["correction"]),
            "train_overshoot_loss": safe_mean(stats["overshoot"]),
            "train_correction_direction_cosine": safe_mean(
                stats["direction_cosine"]
            ),
            "train_overshoot_rate": safe_mean(stats["overshoot_rate"]),
            "train_routed_residual_gradient_norm": safe_mean(
                stats["routed_gradient"]
            ),
            "train_residual_gain_ramp": min(
                1.0, epoch / max(1, args.residual_gain_ramp_epochs)
            ),
            "train_direct_reference_l1": safe_mean(stats["direct_reference"]),
            "train_hybrid_minus_direct_l1": safe_mean(stats["hybrid_minus_direct"]),
            "train_residual_gain_violation_rate": safe_mean(stats["gain_violation"]),
            ...
            "train_direct_rms_clean": safe_mean(stats["direct_rms_clean"]),
            "train_direct_rms_corrupt": safe_mean(stats["direct_rms_corrupt"]),
            "train_residual_rms_clean": safe_mean(stats["residual_rms_clean"]),
            "train_residual_rms_corrupt": safe_mean(stats["residual_rms_corrupt"]),
            "train_residual_rms_max_clean": max(stats["residual_rms_max_clean"], default=0.0),
            "train_residual_rms_max_corrupt": max(stats["residual_rms_max_corrupt"], default=0.0),
            "train_residual_direct_ratio_clean": safe_mean(stats["residual_ratio_clean"]),
            "train_residual_direct_ratio_corrupt": safe_mean(stats["residual_ratio_corrupt"]),
            "train_raw_auxiliary_rms_clean": safe_mean(stats["raw_aux_rms_clean"]),
            "train_raw_auxiliary_rms_corrupt": safe_mean(stats["raw_aux_rms_corrupt"]),
            "train_target_auxiliary_cosine_clean": safe_mean(
                stats["feature_cosine_clean"]
            ),
            "train_target_auxiliary_cosine_corrupt": safe_mean(
                stats["feature_cosine_corrupt"]
            ),
            "val_patient_l1": val["patient_l1"],
            "val_slice_l1": val["slice_l1"],
            "val_residual_off_patient_l1": val["residual_off_patient_l1"],
            "val_hybrid_minus_residual_off_patient_l1": val[
                "hybrid_minus_residual_off_patient_l1"
            ],
            "val_patients_hybrid_better_than_residual_off": val[
                "patients_hybrid_better_than_residual_off"
            ],
            "val_clean_q": val["clean_q"],
            "val_clean_need": val["clean_need"],
            "val_clean_gated_rms": val["clean_gated_rms"],
            "val_clean_direct_rms": val["clean_direct_rms"],
            "val_clean_residual_rms": val["clean_residual_rms"],
            "val_clean_residual_rms_max": val["clean_residual_rms_max"],
            "val_clean_residual_direct_ratio": val["clean_residual_direct_ratio"],
            "val_clean_raw_auxiliary_rms": val["clean_raw_auxiliary_rms"],
            "val_clean_target_auxiliary_cosine": val[
                "clean_target_auxiliary_cosine"
            ],
    ...
    summary = {
        ...
        "fusion_design": args.fusion_design,
        "candidate_name": "rpcrf_h4_final_formal50",
        "need_scope": args.need_scope,
        "completed_epochs": args.epochs,
        "best_epoch": best_epoch,
        "best_val_patient_l1": best_val,
        "best_epoch_val_residual_off_patient_l1": best_history_row[
            "val_residual_off_patient_l1"
        ],
        "best_epoch_val_hybrid_minus_residual_off_patient_l1": best_history_row[
            "val_hybrid_minus_residual_off_patient_l1"
        ],
        "best_epoch_val_patients_hybrid_better_than_residual_off": best_history_row[
            "val_patients_hybrid_better_than_residual_off"
        ],
        "parameter_count": parameter_count,
        "lambda_residual_gain": args.lambda_residual_gain,
        "lambda_correction": args.lambda_correction,
        "lambda_overshoot": args.lambda_overshoot,
        "overshoot_kappa": args.overshoot_kappa,
        "correction_weight_clip": args.correction_weight_clip,
        "residual_reliability_power": args.residual_reliability_power,
        "residual_levels": ["H/4"],
        "correction_gradient_scope": (
            "H/4 complementary residual generator and local gates only"
        ),
        "residual_gain_margin_relative": args.residual_gain_margin_relative,
        "checkpoint_selection_metric": "patient-level clean validation L1",
    }
