...
"""Continue the selected Stage-A QMax arm from epoch 30 through epoch 60.

This runtime extension deliberately leaves the original epoch-1-to-30
training script unchanged, so its structural hashes remain valid. Recovery
must use model_last.pt from the exact original formal output directory.
"""
...
# Extending the already declared training horizon from 30 to 60 is the only
# permitted checkpoint-config difference. All scientific inputs, model
# semantics, optimizer definition and initial LR remain immutable.
CONTINUATION_IMMUTABLE_RESUME_KEYS = tuple(
    key for key in IMMUTABLE_RESUME_KEYS if key != "epochs"
)
...
def main() -> None:
    ...
    if args.learning_rate != 3e-4:
        raise ValueError("Stage-A continuation preserves initial LR=3e-4")
    ...
    if args.run_mode == "formal" and args.epochs != 60:
        raise ValueError("Stage-A continuation must end at epoch 60")
    if args.run_mode != "formal" or args.resume is None:
        raise ValueError(
            "Epoch 31-60 is resume-only and requires --run_mode formal "
            "with --resume"
        )
    ...
    config.update(
        {
            "protocol_version": "QMax-StageA-R8-R2-continue31to60-v1",
            ...
            "continuation_script_sha256": sha256_file(Path(__file__)),
            "code_hashes": hashes,
    ...
    if args.resume:
        ...
        validate_resume_config(
            checkpoint["config"],
            config,
            CONTINUATION_IMMUTABLE_RESUME_KEYS,
        )
        ...
        if start_epoch < 31:
            raise RuntimeError(
                "Stage-A continuation requires a checkpoint at epoch 30 "
                "or later"
            )
    else:
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if validation["patient_l1"] < best_val:
            ...
            save_checkpoint(
                path=output_dir / "model_best_within_60.pt",
                model=model,
                optimizer=optimizer,
                scaler=scaler,
                epoch=epoch,
        ...
        if epoch in (40, 50, 60):
            save_checkpoint(
                path=output_dir / f"epoch{epoch}" / "model_last.pt",
                model=model,
                optimizer=optimizer,
                scaler=scaler,
                epoch=epoch,
    ...
    final_summary = {
        ...
        "best_within_60_epoch": best_epoch,
        "best_within_60_val_patient_l1": best_val,
