...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train the M2-GD v2.1 head-only paired discriminative branch "
            "from the robust V2 Epoch-5 checkpoint."
        )
    )
    ...
    parser.add_argument("--reliability_lr", type=float, default=1e-4)
    ...
    if args.curriculum == "paired3" and args.epochs != 3:
        raise ValueError("paired3 curriculum is pre-registered for exactly three epochs.")
    ...
    transfer_report = load_m2gd_v2_for_v21(
        model,
        args.m2gd_v2_checkpoint,
        map_location=device,
    )
    if int(transfer_report.get("checkpoint_epoch", -1)) != 5:
        raise RuntimeError(
            "Stage-A V2.1 requires the audited V2 Epoch-5 checkpoint; "
            f"received {transfer_report}."
        )
    ...
    source_config = transfer_report.get("checkpoint_config", {})
    provenance_mismatches = {
        key: {"expected": expected, "observed": source_config.get(key)}
        for key, expected in expected_source_config.items()
        if source_config.get(key) != expected
    }
    if provenance_mismatches:
        raise RuntimeError(
            "V2.1 source checkpoint provenance mismatch: "
            f"{provenance_mismatches}"
        )
    print("M2-GD V2 to V2.1 transfer report:")
    ...
    optimizer = torch.optim.Adam(
        ...
        lr=args.reliability_lr,
    )
    parameter_count = sum(parameter.numel() for parameter in model.parameters())
    new_parameter_count = sum(parameter.numel() for parameter in reliability_parameters)
    ...
    config.update(
        {
            ...
            "checkpoint_selection_metric": (
                "last epoch after the complete paired3 discrimination curriculum; "
                "external audit determines mechanism eligibility"
            ),
            "reliability_name": (
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_index, batch in enumerate(train_loader, start=1):
            ...
            loss_recon = 0.5 * (loss_clean_recon + loss_corrupted_recon)
            ...
            # Stage A is deliberately head-only. Reconstruction is monitored but
            # does not participate in the backward objective.
            loss_total = args.lambda_rel * loss_reliability_composite
    ...
    final_summary = {
        ...
        "m2gd_v2_checkpoint": args.m2gd_v2_checkpoint,
        "curriculum": args.curriculum,
