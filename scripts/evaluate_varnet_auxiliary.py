#!/usr/bin/env python3
"""
Formal validation evaluation for B1 Auxiliary PD-assisted PD-FS VarNet.

Evaluates PD-FS only. PD is used as full/high-quality image-domain auxiliary input.

Example:
  python scripts/evaluate_varnet_auxiliary.py \
    --metadata_csv /path/to/reorganised_dataset_split_isambard.csv \
    --output_dir outputs/evaluation/auxiliary_varnet_val/ep30_bs8_lr1e4 \
    --batch_size 8 \
    --R4_checkpoint outputs/varnet_auxiliary/aux_pdfs_R4_ep30_bs8_lr1e4/model_best.pt \
    --R6_checkpoint outputs/varnet_auxiliary/aux_pdfs_R6_ep30_bs8_lr1e4/model_best.pt \
    --R8_checkpoint outputs/varnet_auxiliary/aux_pdfs_R8_ep30_bs8_lr1e4/model_best.pt
"""
...
class ShapeBucketBatchSampler(Sampler[List[int]]):
    """Batch sampler that avoids collating slices with different k-space sizes."""
...
def center_crop_np(x: np.ndarray, shape: Tuple[int, int]) -> np.ndarray:
    ...
    if h == th and w == tw:
        return x
    if h < th or w < tw:
        raise RuntimeError(f"Cannot center-crop array of shape {(h, w)} to larger shape {(th, tw)}")
    top = (h - th) // 2
    left = (w - tw) // 2
...
def l1_metric(target: np.ndarray, pred: np.ndarray) -> float:
    """Target-scaled L1, matching the training/evaluation convention used in this project."""
...
def prepare_batch(batch: Dict, device: torch.device):
    ...
    pd_aux = batch["pd_target_raw"].to(device, non_blocking=True).float()
...
def evaluate_one_R(
    ...
    print("=" * 80)
    print(f"Evaluating B1 AuxPDVarNet R={acceleration}")
    print("=" * 80)
    ...
    per_slice_path = output_dir / f"auxiliary_R{acceleration}_val_per_slice_metrics.csv"
    patient_path = output_dir / f"auxiliary_R{acceleration}_val_patient_level_metrics.csv"
    summary_path = output_dir / f"auxiliary_R{acceleration}_val_summary.csv"
    summary_json_path = output_dir / f"auxiliary_R{acceleration}_val_summary.json"
...
def build_checkpoint_plan(args) -> Dict[int, Path]:
    plan: Dict[int, Path] = {}
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate B1 Auxiliary PD-assisted PD-FS VarNet on validation data.")
    ...
    print("=" * 80)
    print("B1 AuxPDVarNet formal validation evaluation")
    print("=" * 80)
    ...
    combined_summary_path = args.output_dir / "auxiliary_varnet_val_slice_level_summary_R4_R6_R8.csv"
    combined_summary.to_csv(combined_summary_path, index=False)

    combined_slice = pd.concat(all_slice_dfs, ignore_index=True).sort_values(["R", "contrast", "patient_id", "slice_idx"])
    combined_slice_path = args.output_dir / "auxiliary_varnet_val_per_slice_metrics_R4_R6_R8.csv"
    ...
    combined_patient_path = args.output_dir / "auxiliary_varnet_val_patient_level_metrics_R4_R6_R8.csv"
    combined_patient.to_csv(combined_patient_path, index=False)

    print("=" * 80)
    print("Combined B1 auxiliary summary")
    print("=" * 80)
