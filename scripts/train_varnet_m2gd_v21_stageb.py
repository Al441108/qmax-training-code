...
def safe_mean(values: Iterable[float]) -> float:
    # Some paired diagnostics are deliberately undefined for batches that
    # contain only border controls (or only ranked corruptions). Ignore those
    # not-applicable NaNs without masking non-finite optimization losses,
    # which are checked explicitly before backward.
    finite_values = [
        float(value)
        for value in values
...
def paired_margin_loss(
    ...
    return paired_discrimination_loss(
        q_clean=q_clean,
        q_corrupted=q_corrupted,
        corrupted_targets=corrupted_targets,
        records=records,
    )


def alternate_padding_variant(
    clean_pd: torch.Tensor,
    corrupted_pd: torch.Tensor,
    records: Sequence[Dict[str, Any]],
    batch_index: int,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Create a second padding view for matched shift consistency.

    Non-shift samples are copied unchanged and excluded by the returned mask.
    For a shift sample, anatomy, direction and magnitude remain identical; only
    boundary extension changes.
    """
    output = corrupted_pd.clone()
    selected = torch.zeros(
        clean_pd.shape[0],
        device=clean_pd.device,
        dtype=torch.bool,
    )
    modes = ("reflect", "replicate", "zero")
    for sample_index, record in enumerate(records):
        if str(record.get("condition")) != "shift":
            continue
        current = str(record.get("padding_mode"))
        alternatives = [mode for mode in modes if mode != current]
        alternative = alternatives[(batch_index + sample_index) % len(alternatives)]
        output[sample_index] = translate_nonwrapping(
            clean_pd[sample_index],
            int(record.get("dy", 0)),
            int(record.get("dx", 0)),
            alternative,
        )
...
def make_model(args, device: torch.device) -> M2GDv21AuxPDVarNet:
...
def load_stagea_checkpoint(
    model: M2GDv21AuxPDVarNet,
    checkpoint_path: Path,
    audit_decision_path: Path,
    device: torch.device,
    expected_acceleration: int,
    expected_pd_aux_acceleration: int,
) -> Dict[str, Any]:
    audit = json.loads(audit_decision_path.read_text(encoding="utf-8"))
    stagea_gate = audit.get("go_to_fusion_only_calibration")
    if stagea_gate is None:
        # Backward compatibility with the already completed Revision-2 audit.
        stagea_gate = audit.get("go_to_stage_b_joint_finetuning")
    if stagea_gate is not True:
        raise RuntimeError(
            "Stage B is blocked because the Stage-A audit did not pass all "
            "hard guardrails."
        )
    try:
    ...
    if not isinstance(checkpoint, dict):
        raise RuntimeError("Stage-A checkpoint must be a dictionary.")
    config = checkpoint.get("config", {})
    if not isinstance(config, dict):
        raise RuntimeError("Stage-A checkpoint does not contain a valid config.")
    expected = {
        "epoch": 3,
        "acceleration": expected_acceleration,
        "pd_aux_acceleration": expected_pd_aux_acceleration,
        "curriculum": "paired3",
        "num_cascades": 12,
        "pools": 4,
        "chans": 18,
    ...
    observed = {
        ...
        **{key: config.get(key) for key in expected if key != "epoch"},
    }
    mismatch = {
    ...
    if mismatch:
        raise RuntimeError(f"Stage-A checkpoint provenance mismatch: {mismatch}")
    state = checkpoint.get("model_state_dict")
    if not isinstance(state, dict):
        raise RuntimeError("Stage-A checkpoint has no model_state_dict.")
    ...
    if audited_checksum is None:
        raise RuntimeError("Stage-A audit does not record the V2.1 SHA-256.")
    if checksum != audited_checksum:
        raise RuntimeError(
            "Stage-A checkpoint SHA-256 does not match the audited checkpoint."
        )
    return {
        ...
        "checkpoint_config": observed,
        "audit_decision": str(audit_decision_path),
        "audit_passed": True,
        "audit_criteria_count": len(audit.get("criteria", {})),
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Stage-B fusion-only calibration from an audited M2-GD "
            "v2.1 Stage-A checkpoint."
        )
    )
    parser.add_argument("--metadata_csv", required=True)
    parser.add_argument("--m2gd_v21_stagea_checkpoint", required=True)
    parser.add_argument("--stagea_audit_decision", required=True)
    ...
    # This is the number of anatomical pairs.  The actual model forward batch
    # contains 3x this value: clean, corrupted and alternate-padding views.
    ...
    parser.add_argument("--reliability_lr", type=float, default=2e-5)
    parser.add_argument("--fusion_lr", type=float, default=5e-6)
    parser.add_argument("--clean_recon_weight", type=float, default=0.60)
    parser.add_argument("--lambda_rel", type=float, default=0.05)
    parser.add_argument("--rank_weight", type=float, default=1.0)
    parser.add_argument("--padding_weight", type=float, default=0.25)
    ...
    if args.curriculum == "fusioncal3" and args.epochs != 3:
        raise ValueError(
            "fusioncal3 curriculum is pre-registered for exactly three epochs."
        )
    if min(args.lambda_rel, args.rank_weight, args.padding_weight) < 0:
        raise ValueError("Reliability loss weights must be non-negative.")
    if not 0.0 <= args.clean_recon_weight <= 1.0:
        raise ValueError("clean_recon_weight must be within [0,1].")
    ...
    source_report = load_stagea_checkpoint(
        model,
        Path(args.m2gd_v21_stagea_checkpoint),
        Path(args.stagea_audit_decision),
        device,
        args.acceleration,
        args.pd_aux_acceleration,
    )
    print("Audited Stage-A to fusion-only calibration source report:")
    print(json.dumps(source_report, indent=2, default=str))

    reliability_parameters: List[torch.nn.Parameter] = []
    fusion_control_parameters: List[torch.nn.Parameter] = []
    for name, parameter in model.named_parameters():
        is_reliability = "reliability_head" in name
        is_fusion_control = (
            "channel_gate" in name
            or "spatial_gate" in name
            or name.endswith(".alpha")
        )
        parameter.requires_grad = is_reliability or is_fusion_control
        if is_reliability:
            reliability_parameters.append(parameter)
        elif is_fusion_control:
            fusion_control_parameters.append(parameter)
    if not reliability_parameters:
        raise RuntimeError("No V2.1 reliability-head parameters were selected.")
    if not fusion_control_parameters:
        raise RuntimeError("No fusion-control parameters were selected.")
    optimizer = torch.optim.Adam(
        [
            {"params": reliability_parameters, "lr": args.reliability_lr},
            {"params": fusion_control_parameters, "lr": args.fusion_lr},
        ]
    )
    parameter_count = sum(parameter.numel() for parameter in model.parameters())
    new_parameter_count = sum(
        parameter.numel()
        for parameter in (*reliability_parameters, *fusion_control_parameters)
    )
    ...
    config.update(
        {
            ...
            "effective_anatomical_pair_batch_size": (
                args.batch_size * args.grad_accum_steps
            ),
            "effective_forward_image_batch_size": (
                3 * args.batch_size * args.grad_accum_steps
            ),
            "checkpoint_selection_metric": (
                "last epoch after the complete fusioncal3 curriculum; "
                "a new external audit determines final eligibility"
            ),
            "reliability_name": (
                "paired discriminative context-aware reliability score"
            ),
            "stagea_source_report": source_report,
            "trainable_scope": (
                "reliability_head + channel_gate + spatial_gate + alpha; "
                "VarNet, sensitivity, PD encoder and adapters frozen"
            ),
            "training_stage_name": "Stage-B fusion-only calibration",
            "reconstruction_backbone_joint_finetuning": False,
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as file:
        csv.DictWriter(
            ...
            fieldnames=[
                ...
                "train_total_loss",
                "train_recon_l1",
                "train_clean_recon_l1",
                "train_corrupt_recon_l1",
                "train_reliability_bce",
                "train_reliability_composite",
                "train_weighted_reliability",
                "train_pair_margin_loss",
                "train_weighted_pair_margin",
                "train_padding_consistency_loss",
                "train_weighted_padding_consistency",
                "train_rank_violation_rate",
                "train_q_clean",
                "train_q_corrupted",
                "train_q_gap",
                "val_pdfs_patient_l1",
                ...
                "fusion_control_gradient_norm_mean",
                "epoch_seconds",
                "peak_gpu_memory_gb",
            ],
        ).writeheader()
    ...
    best_clean_epoch = 0
    history: List[Dict[str, Any]] = []
    first_eligible_epoch = args.epochs
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        clean_recon_losses: List[float] = []
        corrupt_recon_losses: List[float] = []
        reliability_losses: List[float] = []
        reliability_composite_losses: List[float] = []
        pair_margin_losses: List[float] = []
        padding_consistency_losses: List[float] = []
        rank_violation_rates: List[float] = []
        q_clean_values: List[float] = []
        q_corrupted_values: List[float] = []
        q_gap_values: List[float] = []
        gradient_norms: List[float] = []
        reliability_gradient_norms: List[float] = []
        fusion_control_gradient_norms: List[float] = []
        ...
        for batch_index, batch in enumerate(train_loader, start=1):
            ...
            base_batch_size = int(pd_aux.shape[0])
            alternate_pd, padding_mask = alternate_padding_variant(
                clean_pd=pd_aux,
                corrupted_pd=corrupted.image,
                records=corrupted.records,
                batch_index=batch_index,
            )
            paired_kspace = torch.cat([kspace, kspace, kspace], dim=0)
            paired_mask = torch.cat([mask, mask, mask], dim=0)
            paired_pd = torch.cat([pd_aux, corrupted.image, alternate_pd], dim=0)
            paired_availability = torch.cat(
                [
                    torch.ones_like(corrupted.availability),
                    corrupted.availability,
                    corrupted.availability,
                ],
                dim=0,
            )
            paired_target = torch.cat([target, target, target], dim=0)

            prediction, aux = model(
                pdfs_masked_kspace=paired_kspace,
                mask=paired_mask,
                pd_aux_image=paired_pd,
                pd_available=paired_availability,
                return_aux=True,
                detach_q_for_fusion=False,
            )
            prediction = center_crop(
                prediction,
                crop_h=paired_target.shape[-2],
                crop_w=paired_target.shape[-1],
            )
            clean_prediction = prediction[:base_batch_size]
            corrupted_prediction = prediction[base_batch_size : 2 * base_batch_size]
            loss_clean_recon = l1_per_sample(clean_prediction, target).mean()
            loss_corrupted_recon = l1_per_sample(
                corrupted_prediction,
                target,
            ).mean()
            loss_recon = (
                args.clean_recon_weight * loss_clean_recon
                + (1.0 - args.clean_recon_weight) * loss_corrupted_recon
            )

            q_hat = aux["q_hat"]
            q_logits = aux["q_logits"]
            q_hat_clean = q_hat[:base_batch_size]
            q_hat_corrupted = q_hat[base_batch_size : 2 * base_batch_size]
            q_hat_alternate = q_hat[2 * base_batch_size :]
            logits_clean = q_logits[:base_batch_size]
            logits_corrupted = q_logits[base_batch_size : 2 * base_batch_size]
            clean_reliability_target = torch.ones_like(logits_clean)
            corrupted_reliability_target = (
                corrupted.reliability_target[:, None, :].expand_as(
                    logits_corrupted
                )
            )
            loss_clean_bce = F.binary_cross_entropy_with_logits(
                logits_clean,
                clean_reliability_target,
            )
            corrupt_bce_elements = F.binary_cross_entropy_with_logits(
                logits_corrupted,
                corrupted_reliability_target,
                reduction="none",
            )
            corrupt_sample_weights = torch.tensor(
                [0.5 if record["condition"] == "missing" else 1.0 for record in corrupted.records],
                device=device,
                dtype=corrupt_bce_elements.dtype,
            )[:, None, None]
            loss_corrupted_bce = (
                corrupt_bce_elements * corrupt_sample_weights
            ).sum() / (
                corrupt_sample_weights.expand_as(corrupt_bce_elements).sum().clamp_min(1.0)
            )
            loss_reliability = 0.5 * (loss_clean_bce + loss_corrupted_bce)
            loss_pair_margin, pair_diagnostics = paired_margin_loss(
                q_hat_clean,
                q_hat_corrupted,
            ...
            if bool(padding_mask.any()):
                loss_padding_consistency = F.mse_loss(
                    q_hat_corrupted[padding_mask],
                    q_hat_alternate[padding_mask],
                )
            else:
                loss_padding_consistency = q_hat_corrupted.sum() * 0.0
            loss_reliability_composite = (
                loss_reliability
                + args.rank_weight * loss_pair_margin
                + args.padding_weight * loss_padding_consistency
            )
            # Stage B deliberately reconnects q and the fusion controls to the
            # reconstruction objective while the reconstruction backbone stays
            # frozen.
            loss_total = loss_recon + args.lambda_rel * loss_reliability_composite
            ...
            if should_step:
                ...
                fusion_control_gradient_norms.append(
                    parameter_gradient_norm(
                        model,
                        ("channel_gate", "spatial_gate", ".alpha"),
                    )
                )
            ...
            clean_recon_losses.append(float(loss_clean_recon.item()))
            corrupt_recon_losses.append(float(loss_corrupted_recon.item()))
            reliability_losses.append(float(loss_reliability.item()))
            reliability_composite_losses.append(
                float(loss_reliability_composite.item())
            )
            pair_margin_losses.append(float(loss_pair_margin.item()))
            padding_consistency_losses.append(
                float(loss_padding_consistency.item())
            )
            rank_violation_rates.append(pair_diagnostics["rank_violation_rate"])
            q_clean_values.append(pair_diagnostics["q_clean"])
            q_corrupted_values.append(pair_diagnostics["q_corrupted"])
            q_gap_values.append(pair_diagnostics["q_gap"])
            corrupted_aux = {
                key: aux[key][base_batch_size : 2 * base_batch_size]
                for key in (
                    "q_hat",
                    "q",
            ...
            if batch_index == 1 or batch_index % 25 == 0:
                print(
                    ...
                    f"rank={loss_pair_margin.item():.6f} | "
                    f"pad={loss_padding_consistency.item():.6f} | "
                    f"q_gap={pair_diagnostics['q_gap']:.4f}",
                    flush=True,
                )
        ...
        row = {
            ...
            "train_clean_recon_l1": safe_mean(clean_recon_losses),
            "train_corrupt_recon_l1": safe_mean(corrupt_recon_losses),
            "train_reliability_bce": safe_mean(reliability_losses),
            "train_reliability_composite": safe_mean(reliability_composite_losses),
            "train_weighted_reliability": (
                args.lambda_rel * safe_mean(reliability_composite_losses)
            ),
            "train_pair_margin_loss": safe_mean(pair_margin_losses),
            "train_weighted_pair_margin": (
                args.lambda_rel * args.rank_weight * safe_mean(pair_margin_losses)
            ),
            "train_padding_consistency_loss": safe_mean(padding_consistency_losses),
            "train_weighted_padding_consistency": (
                args.lambda_rel
                * args.padding_weight
                * safe_mean(padding_consistency_losses)
            ),
            "train_rank_violation_rate": safe_mean(rank_violation_rates),
            "train_q_clean": safe_mean(q_clean_values),
            "train_q_corrupted": safe_mean(q_corrupted_values),
            "train_q_gap": safe_mean(q_gap_values),
            ...
            "fusion_control_gradient_norm_mean": safe_mean(
                fusion_control_gradient_norms
            ),
    ...
    final_summary = {
        ...
        "parameter_count": parameter_count,
        "trainable_parameter_count": new_parameter_count,
        "checkpoint_selection_metric": (
            "patient-level mean clean validation PDFS L1 among curriculum-eligible epochs"
        ),
        "m2gd_v21_stagea_checkpoint": args.m2gd_v21_stagea_checkpoint,
        "stagea_audit_decision": args.stagea_audit_decision,
        "curriculum": args.curriculum,
        "training_stage_name": "Stage-B fusion-only calibration",
        "reconstruction_backbone_joint_finetuning": False,
        "trainable_scope": (
