...
def locked_code_hashes() -> Dict[str, str]:
    relative_paths = [
        ...
        "scripts/train_m2_prnf_quality_gain_formal50.py",
        ...
        "src/fft_utils.py",
        "src/masks.py",
        "MULTISEED_CONFIRMATION_PROTOCOL_R8.json",
    ]
    result = {}
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Quality-protected hybrid PRNF formal multi-seed confirmation "
            "supervision."
        )
    )
    ...
    parser.add_argument("--run_stage", required=True, choices=["quality_gain_formal50"])
    parser.add_argument(
    ...
    if args.variant != "prnf_no_need":
        raise ValueError("Quality-gain formal confirmation must use prnf_no_need")
    if args.fusion_design != "hybrid_direct_residual":
        raise ValueError("Quality-gain formal confirmation requires hybrid_direct_residual")
    if args.need_scope != "residual":
        raise ValueError("Quality-gain formal confirmation requires need_scope=residual")
    if args.epochs != 50:
        raise ValueError("Quality-gain formal confirmation requires exactly 50 epochs")
