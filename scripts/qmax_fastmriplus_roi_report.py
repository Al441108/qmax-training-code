...
def save_png_figure(fig: plt.Figure, stem: Path) -> str:
    """Save a delivery PNG without creating redundant vector/TIFF copies."""
    stem.parent.mkdir(parents=True, exist_ok=True)
...
def render_representative_plate(
    ...
    for selection in selected["cases"]:
        ...
        fig, axes = plt.subplots(2, 3, figsize=(12.6, 7.8), squeeze=False)
        titles = (
            ("Reference + box", "Zero-filled + box", "QMax-Full + box"),
            ("Reference crop", "Zero-filled crop", "QMax-Full crop"),
        )
        for column, image in enumerate((target, zf, qmax)):
            axes[0, column].imshow(image, cmap="gray", vmin=low, vmax=high)
            draw_boxes(axes[0, column], boxes)
        for column, image in enumerate((target, zf, qmax)):
            axes[1, column].imshow(
                image[top:bottom, left:right], cmap="gray", vmin=low, vmax=high
            )
            draw_boxes(axes[1, column], boxes, crop=crop)
        for row_index in range(2):
            for column in range(3):
                axes[row_index, column].set_title(
                    titles[row_index][column], fontsize=13, fontweight="bold", pad=7
                )
        ...
        fig.suptitle(
            f"{selection['selection_position'].capitalize()} lesion L1 improvement | {pathology}\n"
            f"Case {key[0][:8]} · slice {key[1]}",
            fontsize=16,
            fontweight="bold",
            y=0.985,
        )
        fig.subplots_adjust(
            left=0.025, right=0.99, top=0.88, bottom=0.10, wspace=0.07, hspace=0.25
        )
        position = str(selection["selection_position"]).lower()
        outputs[position] = save_png_figure(
            fig, output / f"representative_lesion_plate_{position}"
        )
        plt.close(fig)
    return outputs
...
def render_error_plate(
    ...
    for selection in selected["cases"]:
        ...
        grid = fig.add_gridspec(
            1, 3, width_ratios=(1.0, 1.0, 0.045),
            left=0.06, right=0.94, top=0.80, bottom=0.12, wspace=0.09,
        )
        axes = [fig.add_subplot(grid[0, 0]), fig.add_subplot(grid[0, 1])]
        colorbar_axis = fig.add_subplot(grid[0, 2])
        for column, error in enumerate((zf_error, qmax_error)):
            image = axes[column].imshow(error, cmap="magma", vmin=0, vmax=high)
            axes[column].axis("off")
        axes[0].set_title("Zero-filled absolute-error crop", fontsize=13, fontweight="bold", pad=8)
        axes[1].set_title("QMax-Full absolute-error crop", fontsize=13, fontweight="bold", pad=8)
        colorbar = fig.colorbar(image, cax=colorbar_axis)
        colorbar.ax.tick_params(labelsize=10)
        colorbar.set_label("Absolute error", fontsize=11, labelpad=8)
        pathology = "; ".join(metadata["labels"])
        fig.suptitle(
            f"{selection['selection_position'].capitalize()} lesion L1 improvement | {pathology}\n"
            f"Case {key[0][:8]} · slice {key[1]}",
        ...
        fig.text(
            0.5, 0.035, "Shared colour scale within this case", ha="center",
            fontsize=10, color="#444444",
        )
        position = str(selection["selection_position"]).lower()
        outputs[position] = save_png_figure(
            fig, output / f"representative_lesion_errors_{position}"
        )
        plt.close(fig)
    return outputs
