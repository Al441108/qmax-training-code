...
PROTOCOL_VERSION = "M2-PRNF-R8-v1.5.0-quality-gain-comparison-audited"
...
EVALUATOR_HOTFIX_ID = "quality-gain-five-arm-evaluator-v1"
PILOT_ARMS = (
    "legacy_local_direct",
    "global_direct",
    "residual_only",
...
def build_model(
    ...
    expected_stage = "quality_gain_pilot" if is_gain_candidate else "pilot"
    ...
    expected_code_hashes = (
        gain_locked_code_hashes() if is_gain_candidate
    ...
    model.eval()
    observed_name = arm if is_gain_candidate else fusion_design
    return model, observed_name, config, selected
...
FAIRNESS_KEYS = (
    ...
    "metadata_sha256", "optimizer", "gradient_clip_norm", "runtime_versions",
)
...
def main():
    ...
    for arm, path in checkpoints.items():
        ...
        if arm == GAIN_ARM:
            with residual_scale_override(model, 0.0):
                slice_rows += evaluate_mode(
                    model, GAIN_RESIDUAL_OFF, loaders["full_clean"],
    ...
    residual_incremental_gain_tests = [
        bootstrap_improvement(
            patient_rows, GAIN_ARM, GAIN_RESIDUAL_OFF, "full_clean", "correct",
            metric, args.bootstrap_resamples, args.seed
        )
        for metric in METRICS
    ]
    comparisons.extend(residual_incremental_gain_tests)
    ...
    gain_l1_test = next(
        test for test in residual_incremental_gain_tests if test["metric"] == "l1"
    )
    gain_mechanism_status = (
        "eligible" if gain_l1_test["ci95_low"] > 0.0
        else "borderline" if gain_l1_test["mean_improvement"] > 0.0
        else "rejected"
    )
    arm_decisions[GAIN_ARM]["same_checkpoint_residual_incremental_gain"] = {
        "status": gain_mechanism_status,
        "tests": residual_incremental_gain_tests,
        "interpretation": (
            "Positive values mean residual-on is better than the identical "
            "checkpoint evaluated with residual_scale=0."
        ),
    }
    if gain_mechanism_status == "rejected":
        arm_decisions[GAIN_ARM]["qualification"] = "rejected"
    elif (
        gain_mechanism_status == "borderline"
        and arm_decisions[GAIN_ARM]["qualification"] == "eligible"
    ):
        arm_decisions[GAIN_ARM]["qualification"] = "borderline"
    ...
    decision = {
        ...
        "recommendation_rule": (
            ...
            "The quality-gain candidate must additionally show a positive "
            "same-checkpoint residual-on versus residual-off clean L1 effect. "
    ...
    audit = {
        ...
        "evaluator_hotfix": {
            ...
            "scope": (
                "Five-arm locked comparison with an additional same-checkpoint "
                "residual-on versus residual-off causal intervention for the "
                "quality-gain candidate."
            ),
