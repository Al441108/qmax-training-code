...
IMMUTABLE_RESUME_KEYS = (
    "variant", "fusion_design", "need_scope", "residual_scale", "run_stage",
    "metadata_csv", "metadata_sha256", "condition_manifest",
    ...
    "residual_gain_ramp_epochs", "seed",
...
@contextmanager
def residual_scale_override(model: M2PRNFFusionPilotVarNet, value: float):
...
def validate_resume_config(previous: Dict[str, Any], current: Dict[str, Any]) -> None:
    """Validate ordinary resumes and the audited continuation transitions."""
    mismatches = {}
    for key in IMMUTABLE_RESUME_KEYS:
        if current.get("run_stage") == "quality_gain_extension_15_to_50":
            if key == "run_stage" and previous.get(key) in {
                "quality_gain_pilot", "quality_gain_extension_15_to_50"
            }:
                continue
            if key == "epochs" and int(previous.get(key, -1)) in {15, 50}:
                continue
        if current.get("run_stage") == "posthoc_low_lr_50_to_60":
            if key == "run_stage" and previous.get(key) in {
                "quality_gain_pilot",
                "quality_gain_extension_15_to_50",
                "posthoc_low_lr_50_to_60",
            }:
                continue
            if key == "epochs" and int(previous.get(key, -1)) in {15, 50, 60}:
                continue
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Audited continuation of the quality-protected hybrid PRNF model "
            "from epoch 15 to a total budget of 50 epochs."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Where the target-need factor acts; pilot arms use prnf_no_need.",
    )
    ...
    parser.add_argument("--batch_size", type=int, default=4, help="Anatomical pairs; forward batch is 2x")
    ...
    if args.variant != "prnf_no_need":
        raise ValueError("Quality-gain pilot must use prnf_no_need")
    if args.fusion_design != "hybrid_direct_residual":
        raise ValueError("Quality-gain pilot requires hybrid_direct_residual")
    if args.need_scope != "residual":
        raise ValueError("Quality-gain pilot requires need_scope=residual")
    if args.run_stage == "quality_gain_pilot":
        if args.epochs != 15:
            raise ValueError("Quality-gain pilot requires exactly 15 epochs")
    elif args.run_stage == "quality_gain_extension_15_to_50":
        if args.epochs != 50:
            raise ValueError("Quality-gain continuation requires a total of 50 epochs")
    elif args.run_stage == "posthoc_low_lr_50_to_60":
        if args.epochs != 60:
            raise ValueError("The post-hoc extension target is 60 total epochs")
        if args.learning_rate != 3e-5:
            raise ValueError("The post-hoc extension requires learning_rate=3e-5")
        if not args.resume:
    ...
    config.update(
        {
            ...
            "fusion_hypothesis": (
                "make the bounded complementary residual causally improve the "
                "same model's residual-off direct reconstruction while retaining "
                "pair-aware reliability protection"
            ),
    ...
    if args.run_stage == "quality_gain_extension_15_to_50":
        config.update(
            {
                "protocol_version": (
                    "M2-PRNF-R8-v1.5.0-quality-gain-continuation-15-to-50-audited"
                ),
                "continuation_source_total_epochs": 15,
                "continuation_target_total_epochs": 50,
                "continuation_script": str(Path(__file__).resolve()),
                "continuation_script_sha256": sha256_file(Path(__file__).resolve()),
                "continuation_semantics": (
                    "Exact optimizer/RNG/sampler/corruption-audit continuation from "
                    "the locked quality-protected hybrid epoch-15 model_last checkpoint"
                ),
            }
        )
    elif args.run_stage == "posthoc_low_lr_50_to_60":
    ...
    if args.resume:
        ...
        if args.run_stage == "posthoc_low_lr_50_to_60":
            source_epoch = int(checkpoint["epoch"])
            if source_epoch < 50 or source_epoch >= 60:
                raise RuntimeError(
                    "Expected a continuation checkpoint from epoch 50-59, "
                    f"got {source_epoch}"
                )
            if source_epoch == 50:
                ...
                if observed != expected:
                    raise RuntimeError(
                        "Invalid Hybrid-gain epoch-50 source:\n"
                        + json.dumps(
        ...
        if args.run_stage == "quality_gain_extension_15_to_50":
            ...
            source_epoch = int(checkpoint["epoch"])
            if source_epoch < 15:
                raise RuntimeError("Continuation source has not completed epoch 15")
            if source_epoch == 15:
                expected_source = {
                    "variant": "prnf_no_need",
                    "fusion_design": "hybrid_direct_residual",
                ...
                if observed_source != expected_source:
                    raise RuntimeError(
                        "Invalid quality-gain pilot continuation source:\n"
                        + json.dumps(
                            {"expected": expected_source, "observed": observed_source},
                            indent=2,
                        )
                    )
            elif source_config.get("run_stage") != "quality_gain_extension_15_to_50":
                raise RuntimeError(
                    "Post-epoch-15 resume is not an audited quality-gain extension checkpoint"
                )
            previous_extension_hash = source_config.get("continuation_script_sha256")
            if (
                previous_extension_hash is not None
                and previous_extension_hash != config["continuation_script_sha256"]
            ):
                raise RuntimeError("Continuation script changed during the extension")
        ...
        if args.run_stage == "quality_gain_extension_15_to_50":
            extension_config_path = output_dir / "extension_config.json"
            if extension_config_path.is_file():
                installed_extension = json.loads(
                    extension_config_path.read_text(encoding="utf-8")
                )
                if installed_extension != config:
                    raise RuntimeError("Installed extension_config.json has changed")
            else:
        ...
        if args.run_stage == "posthoc_low_lr_50_to_60":
            ...
            if observed_lrs != [3e-5]:
                raise RuntimeError(
                    f"Failed to apply continuation LR: {observed_lrs}"
                )
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch_index, batch in enumerate(train_loader, start=1):
            ...
            ) + gain_ramp * args.lambda_residual_gain * residual_gain_loss
            ...
            if batch_index == 1 or batch_index % 25 == 0:
                print(
                    ...
                    f"gain={residual_gain_loss.item():.6f} "
    ...
    summary = {
        ...
        "fusion_design": args.fusion_design,
        "candidate_name": "quality_protected_hybrid_gain",
        "need_scope": args.need_scope,
        ...
        summary.update(
            {
                ...
                "continued_from_epoch": 15,
                "continuation_script_sha256": config[
