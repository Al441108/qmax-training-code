...
"""Continue a frozen QMax-Full replication from epoch 30 through epoch 60."""
...
def main() -> None:
    ...
    if args.seed not in ALLOWED_REPLICATION_SEEDS:
        raise ValueError(
            "Replication seed must be one of "
            f"{ALLOWED_REPLICATION_SEEDS}, got {args.seed}"
        )
    if args.qmax_variant != "qmax_full":
        raise ValueError("Replication package trains QMax-Full only")
    ...
    config.update(
        {
            "protocol_version": (
                "QMax-StageA-R8-R2-multiseed-continue31to60-v1"
            ),
