...
def main():
    ...
    summary = (
        ...
        .agg(
            ...
            NMSE_iqr_low=("NMSE", q25),
            NMSE_iqr_high=("NMSE", q75),

            PSNR_mean=("PSNR", "mean"),
            PSNR_median=("PSNR", "median"),
            PSNR_iqr_low=("PSNR", q25),
            PSNR_iqr_high=("PSNR", q75),

            SSIM_mean=("SSIM", "mean"),
            SSIM_median=("SSIM", "median"),
            SSIM_iqr_low=("SSIM", q25),
            SSIM_iqr_high=("SSIM", q75),
            ...
            actual_acceleration_mean=("actual_R", "mean"),
            actual_acceleration_std=("actual_R", "std"),
        )
        .reset_index()
    )

    summary = summary.rename(columns={"acceleration": "R"})
    summary["contrast"] = summary["contrast"].replace({"PDFS": "PD-FS"})

    summary_csv = out_dir / "zero_filled_summary.csv"
    summary.to_csv(summary_csv, index=False)

    aligned_summary_csv = out_dir / "zero_filled_val_slice_level_summary_R4_R6_R8.csv"
    summary.to_csv(aligned_summary_csv, index=False)
