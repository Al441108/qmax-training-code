...
def prepare_aux_batch(batch, device: torch.device):
    ...
    if pd_aux.ndim != 3:
        raise RuntimeError(f"Expected PD auxiliary [B,H,W], got {pd_aux.shape}")
...
def evaluate(model, loader, device, max_val_batches=None):
    ...
    for batch_index, batch in enumerate(loader, start=1):
        ...
        pdfs_kspace, mask, pd_aux, pdfs_target = prepare_aux_batch(
            batch=batch,
            device=device,
        )
        ...
        if not torch.isfinite(pdfs_losses).all():
            raise RuntimeError("Non-finite PDFS validation loss")

        batch_size = pdfs_target.shape[0]
    ...
    results = {
        ...
        "num_slices": len(pdfs_all),
        "num_edge_slices": len(pdfs_edge),
        "num_central_slices": len(pdfs_central),
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="B1 Auxiliary PD R=2 assisted PD-FS VarNet training."
    )
    ...
    parser.add_argument(
        ...
        choices=[4, 6, 8],
        required=True,
        help="Target PD-FS acceleration factor.",
    )
    ...
    parser.add_argument(
        ...
        default=2,
        help=(
            "Acceleration factor used to generate the zero-filled RSS "
            "PD auxiliary image. Main protocol setting: R=2."
        ),
    )
    ...
    parser.add_argument(
        "--resume",
        default=None,
        help="Path to model_last.pt for resuming auxiliary training.",
    )
    ...
    print("=" * 80)
    print("B1 Auxiliary PD-assisted PD-FS VarNet training")
    print("=" * 80)
    print("Device:", device)
    if device.type == "cuda":
    ...
    print("Target contrast: PD-FS")
    print("Auxiliary input: zero-filled lightly undersampled PD image")
    print("=" * 80)
    ...
    config.update(
        {
            ...
            "pd_aux_acceleration": args.pd_aux_acceleration,
            "mask": "Gaussian variable-density 1D Cartesian, PD-FS target stream only",
            "model": "B1 AuxPDVarNet: PD-FS VarNet stream + full PD image auxiliary regulariser channel",
        ...
        "auxiliary_pd_acceleration": args.pd_aux_acceleration,
            "checkpoint_selection_metric": "validation PDFS overall L1",
        }
    )
    ...
    if resume_path is not None:
        ...
        print("Previous best validation PDFS L1:", best_val)
        print("=" * 80)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch_index, batch in enumerate(train_loader, start=1):
            ...
            if not torch.isfinite(loss):
                raise RuntimeError(f"Non-finite PDFS loss at epoch {epoch}, batch {batch_index}")
            ...
            train_pdfs_losses.append(float(loss.item()))
        ...
        print(
            ...
            f"train_pdfs={epoch_row['train_pdfs_l1']:.6f} | "
        ...
        improved = val_results["pdfs_overall_l1"] < best_val

        if improved:
            best_val = float(val_results["pdfs_overall_l1"])
            best_epoch = epoch
        ...
        if improved:
            ...
            print(
                f"Saved best checkpoint: epoch={epoch}, val_pdfs={best_val:.6f}",
                flush=True,
            )
    ...
    saved_best_value = float(
        best_checkpoint["validation_results"]["pdfs_overall_l1"]
    )
    reloaded_best_value = float(
        reload_results["pdfs_overall_l1"]
    )
    ...
    print("=" * 80)
    print("B1 Auxiliary PD-assisted PD-FS VarNet training completed")
    print("=" * 80)
