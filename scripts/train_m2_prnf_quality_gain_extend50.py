...
def canonical_json(value: Any) -> Any:
    """Return a JSON-normalised value for semantic config comparison.

    PyTorch checkpoints retain tuples, whereas the mirrored JSON configuration
    necessarily reloads those containers as lists.  Normalising both sides
    prevents false resume mismatches without relaxing any value-level check.
    """
    return json.loads(json.dumps(value, sort_keys=True))


def validate_resume_config(previous: Dict[str, Any], current: Dict[str, Any]) -> None:
    """Validate resumes using semantic JSON equality."""
    mismatches = {}
    for key in IMMUTABLE_RESUME_KEYS:
        if current.get("run_stage") == "quality_gain_extension_15_to_50":
            ...
            if key == "epochs" and int(previous.get(key, -1)) in {15, 50}:
                continue
        if canonical_json(previous.get(key)) != canonical_json(current.get(key)):
...
def main() -> None:
    ...
    parser.add_argument(
        "--run_stage",
        required=True,
        choices=["quality_gain_pilot", "quality_gain_extension_15_to_50"],
    )
    parser.add_argument(
    ...
    if args.resume:
        ...
        if args.run_stage == "quality_gain_extension_15_to_50":
            ...
            if extension_config_path.is_file():
                ...
                if canonical_json(installed_extension) != canonical_json(config):
