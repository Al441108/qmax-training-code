...
def install_amp_diagnostic_quantile_compatibility() -> None:
    ...
    The frozen fusion controller computes need percentiles only for logging.
    CUDA quantile in the cluster PyTorch build rejects half and bfloat16
    tensors. This idempotent compatibility shim changes only that detached
    diagnostic reduction; reconstruction tensors, losses and gradients are
    untouched.
    """
    ...
    compatible_quantile._qmax_original_quantile = original
...
def make_grad_scaler(enabled: bool):
    ...
    # PyTorch 2.4+ moved GradScaler to torch.amp. Keep a fallback so the
    # frozen scripts remain usable on older cluster environments.
    try:
...
def code_hashes(project_root: Path) -> Dict[str, str]:
    paths = (
        ...
        "src/fft_utils.py",
        "src/masks.py",
        "scripts/generate_qmax_multiseed_init.py",
        "scripts/qmax_multiseed_common.py",
        "scripts/train_qmax_full_multiseed_epoch1to30.py",
        "scripts/train_qmax_full_multiseed_continue31to60.py",
        "scripts/preflight_qmax_multiseed.py",
        "scripts/run_qmax_multiseed_amp_safe.py",
        "QMAX_FULL_MULTISEED_PROTOCOL_R8.json",
    )
    output = {}
