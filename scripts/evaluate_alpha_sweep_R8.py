#!/usr/bin/env python3
"""
R=8 prediction-level alpha sweep for a PD R=2 auxiliary PD-FS reconstruction model.
...
def summarise_alpha(df: pd.DataFrame) -> pd.DataFrame:
    rows = []

    for (condition, alpha), g in df.groupby(["condition", "alpha"]):
        row = {
            "model": "alpha_fused_single_aux",
            "condition": condition,
            "alpha": float(alpha),
            "R": 8,
            "pd_aux_R": 2,
    ...
    return pd.DataFrame(rows).sort_values(["condition", "alpha"])


def compute_alpha_paired_delta(
    alpha_df: pd.DataFrame,
    robustness_df: pd.DataFrame,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    base = robustness_df[robustness_df["condition"] == "no_auxiliary"].copy()
    base = base[["patient_id", "slice_idx", "R", *METRICS]].rename(
        columns={m: f"{m}_single" for m in METRICS}
    )

    merged = alpha_df.merge(
        base,
        on=["patient_id", "slice_idx", "R"],
        how="inner",
    ...
    for metric in METRICS:
        if HIGHER_IS_BETTER[metric]:
            merged[f"{metric}_delta_vs_single"] = (
                merged[metric] - merged[f"{metric}_single"]
            )
        else:
            merged[f"{metric}_delta_vs_single"] = (
                merged[f"{metric}_single"] - merged[metric]
            )
        merged[f"{metric}_worse_than_single"] = (
            merged[f"{metric}_delta_vs_single"] < 0
        )

    summary_rows = []
    for (condition, alpha), g in merged.groupby(["condition", "alpha"]):
        row = {
            "model": "alpha_fused_single_aux",
            "condition": condition,
            "alpha": float(alpha),
        ...
        for metric in METRICS:
            ...
            row[f"{metric}_pct_worse_than_single"] = (
                float(np.mean(worse) * 100.0) if len(worse) else float("nan")
            )

        summary_rows.append(row)

    summary = pd.DataFrame(summary_rows).sort_values(["condition", "alpha"])
    return merged, summary


def run_alpha_endpoint_sanity_checks(
    alpha_df: pd.DataFrame,
    robustness_df: pd.DataFrame,
    atol: float = 1e-10,
) -> None:
    single = robustness_df[robustness_df["condition"] == "no_auxiliary"][
        ["patient_id", "slice_idx", *METRICS]
    ].copy()

    alpha_zero = alpha_df[alpha_df["alpha"] == 0.0].copy()
    merged_zero = alpha_zero.merge(
        single,
        on=["patient_id", "slice_idx"],
        suffixes=("_alpha", "_single"),
        validate="many_to_one",
    )

    for metric in METRICS:
        a = merged_zero[f"{metric}_alpha"].to_numpy()
        b = merged_zero[f"{metric}_single"].to_numpy()
        if not np.allclose(a, b, rtol=0.0, atol=atol, equal_nan=True):
            diff = np.nanmax(np.abs(a - b))
            raise RuntimeError(
                f"Alpha endpoint sanity check failed: alpha=0 differs from single for "
                f"{metric}; max_abs_diff={diff}"
            )

    aux_conditions = robustness_df[
        robustness_df["condition"] != "no_auxiliary"
    ][["patient_id", "slice_idx", "condition", *METRICS]].copy()

    alpha_one = alpha_df[alpha_df["alpha"] == 1.0].copy()
    merged_one = alpha_one.merge(
        aux_conditions,
        on=["patient_id", "slice_idx", "condition"],
        suffixes=("_alpha", "_aux"),
        validate="one_to_one",
    )

    for metric in METRICS:
        a = merged_one[f"{metric}_alpha"].to_numpy()
        b = merged_one[f"{metric}_aux"].to_numpy()
        if not np.allclose(a, b, rtol=0.0, atol=atol, equal_nan=True):
            diff = np.nanmax(np.abs(a - b))
            raise RuntimeError(
                f"Alpha endpoint sanity check failed: alpha=1 differs from auxiliary "
                f"prediction for {metric}; max_abs_diff={diff}"
            )

    print("Alpha endpoint sanity checks passed: alpha=0 matches single, alpha=1 matches auxiliary.")


def save_alpha_plots(alpha_summary: pd.DataFrame, delta_summary: pd.DataFrame, output_dir: Path) -> None:
    try:
        import matplotlib.pyplot as plt
    except Exception as exc:
        print(f"WARNING: matplotlib unavailable; skipping alpha plots: {exc}")
        return

    plot_specs = [
        ("SSIM_median", "Median SSIM", "alpha_vs_ssim.png"),
        ("L1_median", "Median target-scaled L1", "alpha_vs_l1.png"),
    ]

    for value_col, ylabel, filename in plot_specs:
        plt.figure(figsize=(8, 5))
        for condition, g in alpha_summary.groupby("condition"):
            g = g.sort_values("alpha")
            plt.plot(g["alpha"], g[value_col], marker="o", label=condition)
        plt.xlabel("Auxiliary fusion weight alpha")
        plt.ylabel(ylabel)
        plt.xticks(ALPHAS)
        plt.grid(alpha=0.25)
    ...
    plt.figure(figsize=(8, 5))
    for condition, g in delta_summary.groupby("condition"):
        g = g.sort_values("alpha")
        plt.plot(
            g["alpha"],
            g["SSIM_pct_worse_than_single"],
            marker="o",
            label=condition,
        )
    plt.xlabel("Auxiliary fusion weight alpha")
    plt.ylabel("Slices worse than single in SSIM (%)")
    plt.xticks(ALPHAS)
    plt.grid(alpha=0.25)
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(output_dir / "alpha_vs_pct_worse_ssim.png", dpi=300)
    plt.close()


@torch.no_grad()
def main():
    parser = argparse.ArgumentParser(description="R=8 robustness test and prediction-level alpha sweep for PD R=2 auxiliary model.")
    ...
    print("=" * 80)
    print("AuxPDVarNet PD R=2 -> PD-FS R=8 robustness evaluation + alpha sweep")
    print("=" * 80)
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        for alpha in ALPHAS:
            fused_pred = (1.0 - float(alpha)) * single_pred + float(alpha) * pred_correct
            alpha_rows.extend(
                rows_for_prediction(
                    fused_pred,
                    pdfs_target,
                    batch,
                    "alpha_fused_single_aux",
                    "correct_pdR2_aux",
                    alpha=alpha,
                )
            )
        ...
        for alpha in ALPHAS:
            fused_pred = (1.0 - float(alpha)) * single_pred + float(alpha) * pred_zero
            alpha_rows.extend(
                rows_for_prediction(
                    fused_pred,
                    pdfs_target,
        ...
        for shift in args.shift_pixels:
            ...
            shift_condition = f"shifted_pd_aux_{int(shift)}px_zero_pad"
            all_rows.extend(
                rows_for_prediction(
                    pred_shift,
                    pdfs_target,
            ...
            for alpha in ALPHAS:
                fused_pred = (1.0 - float(alpha)) * single_pred + float(alpha) * pred_shift
                alpha_rows.extend(
                    rows_for_prediction(
                        fused_pred,
                        pdfs_target,
        ...
        for alpha in ALPHAS:
            fused_pred = (1.0 - float(alpha)) * single_pred + float(alpha) * pred_wrong
            alpha_rows.extend(
                rows_for_prediction(
                    fused_pred,
                    pdfs_target,
    ...
    alpha_summary = summarise_alpha(alpha_df)
    alpha_delta_df, alpha_delta_summary = compute_alpha_paired_delta(alpha_df, df)
    run_alpha_endpoint_sanity_checks(alpha_df, df)
    ...
    alpha_patient_df = (
        alpha_df.groupby(
            ["patient_id", "model", "condition", "alpha", "R", "pd_aux_R"],
            as_index=False,
        )[METRICS]
        .median()
        .sort_values(["condition", "alpha", "patient_id"])
    )

    alpha_patient_delta_df = (
        alpha_delta_df.groupby(
            ["patient_id", "model", "condition", "alpha", "baseline_condition", "R", "pd_aux_R"],
            as_index=False,
        )[[f"{m}_delta_vs_single" for m in METRICS]]
        .median()
        .sort_values(["condition", "alpha", "patient_id"])
    ...
    alpha_per_slice_path = args.output_dir / "alpha_sweep_per_slice.csv"
    alpha_summary_path = args.output_dir / "alpha_sweep_summary.csv"
    alpha_patient_path = args.output_dir / "alpha_sweep_per_volume.csv"
    alpha_delta_path = args.output_dir / "alpha_sweep_delta_vs_single.csv"
    alpha_delta_summary_path = args.output_dir / "alpha_sweep_delta_summary.csv"
    alpha_patient_delta_path = args.output_dir / "alpha_sweep_patient_delta_vs_single.csv"
    alpha_json_path = args.output_dir / "alpha_sweep_summary.json"
    ...
    alpha_df.to_csv(alpha_per_slice_path, index=False)
    alpha_summary.to_csv(alpha_summary_path, index=False)
    alpha_patient_df.to_csv(alpha_patient_path, index=False)
    alpha_delta_df.to_csv(alpha_delta_path, index=False)
    alpha_delta_summary.to_csv(alpha_delta_summary_path, index=False)
    alpha_patient_delta_df.to_csv(alpha_patient_delta_path, index=False)

    save_alpha_plots(alpha_summary, alpha_delta_summary, args.output_dir)
        ...
        json.dump(
            {
                ...
                "alpha_summary": alpha_summary.to_dict(orient="records"),
                "alpha_delta_summary": alpha_delta_summary.to_dict(orient="records"),
            },
            f,
            indent=2,
        )
    ...
    print("=" * 80)
    print("Alpha sweep paired delta vs single")
    print("Positive delta means better than single for all metrics.")
    print("=" * 80)
    print(alpha_delta_summary.to_string(index=False))
    print(f"Saved alpha per-slice: {alpha_per_slice_path}")
    print(f"Saved alpha summary: {alpha_summary_path}")
    print(f"Saved alpha per-volume: {alpha_patient_path}")
    print(f"Saved alpha deltas: {alpha_delta_path}")
    print(f"Saved alpha delta summary: {alpha_delta_summary_path}")
    print(f"Saved alpha patient deltas: {alpha_patient_delta_path}")
    print(f"Saved alpha JSON: {alpha_json_path}")
    print("=" * 80)
