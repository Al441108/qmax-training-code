...
def restore_rng_state(state: Dict[str, Any]) -> None:
    ...
    torch.set_rng_state(
        as_cpu_byte_tensor(state["torch_cpu"])
    )

    if torch.cuda.is_available():
        cuda_states = [
            as_cpu_byte_tensor(value)
            for value in state["torch_cuda"]
        ]
        if len(cuda_states) != torch.cuda.device_count():
            raise RuntimeError(
                "CUDA RNG-state/device-count mismatch: "
                f"states={len(cuda_states)}, "
...
def canonical_json(value: Any) -> Any:
    """Normalise checkpoint and JSON values before protocol comparison.

    PyTorch checkpoints preserve tuples, while config.json restores them as
    lists.  Canonical JSON equality prevents false mismatches without relaxing
    any value-level check.
    """
    return json.loads(json.dumps(value, sort_keys=True, allow_nan=False))


def validate_resume_config(previous: Dict[str, Any], current: Dict[str, Any]) -> None:
    """Validate either an ordinary resume or the audited 15->50 transition."""
    mismatches = {}
    for key in IMMUTABLE_RESUME_KEYS:
        if current.get("run_stage") == "pilot_extension_15_to_50":
            ...
            if key == "epochs" and int(previous.get(key, -1)) in {15, 50}:
                continue
        previous_value = canonical_json(previous.get(key))
        current_value = canonical_json(current.get(key))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=["pilot", "formal", "pilot_extension_15_to_50"],
    )
    parser.add_argument(
