...
def validate_resume_config(previous: Dict[str, Any], current: Dict[str, Any]) -> None:
    """Validate an ordinary resume or one of the audited continuations."""
    mismatches = {}
    for key in IMMUTABLE_RESUME_KEYS:
        if current.get("run_stage") == "pilot_extension_15_to_50":
            if key == "run_stage" and previous.get(key) in {
                "pilot", "pilot_extension_15_to_50"
            }:
                continue
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fair from-scratch fusion-operator pilot and final PRNF ablations."
    )
    ...
    parser.add_argument(
        ...
        choices=[
            "pilot", "formal", "pilot_extension_15_to_50",
            "posthoc_low_lr_50_to_60",
        ],
    )
    parser.add_argument(
    ...
    if args.run_stage == "pilot":
        if args.variant != "prnf_no_need":
            raise ValueError("Fusion pilot must use prnf_no_need to isolate fusion design")
        if args.epochs != 15:
            raise ValueError("Pre-registered fusion pilot requires exactly 15 epochs")
    elif args.run_stage == "pilot_extension_15_to_50":
        if args.variant != "prnf_no_need":
            raise ValueError("The audited extension requires prnf_no_need")
        if args.fusion_design != "global_direct":
            raise ValueError("The audited extension requires global_direct")
        if args.need_scope != "residual":
            raise ValueError("The pilot extension must preserve need_scope=residual")
        if args.epochs != 50:
            raise ValueError("The audited extension target is exactly 50 total epochs")
        if not args.resume:
            raise ValueError("The audited extension requires an epoch-15 resume checkpoint")
    elif args.run_stage == "posthoc_low_lr_50_to_60":
        if args.variant != "prnf_no_need":
            raise ValueError("The post-hoc extension requires prnf_no_need")
        if args.fusion_design != "global_direct":
            raise ValueError("The post-hoc extension requires global_direct")
        if args.need_scope != "residual":
            raise ValueError("The post-hoc extension requires need_scope=residual")
        if args.epochs != 60:
        ...
        raise ValueError("Formal PRNF retraining requires exactly 50 epochs")
    ...
    config.update(
        {
            ...
            "fusion_hypothesis": (
                "learn complementary PD-to-PD-FS residuals while retaining "
    ...
    if args.run_stage == "pilot_extension_15_to_50":
        config.update(
            {
                "protocol_version": (
                ...
                "continuation_semantics": (
                    "Exact optimizer/RNG/sampler continuation from the locked "
                    "global-direct prnf_no_need epoch-15 model_last checkpoint"
                ),
            }
        )
    elif args.run_stage == "posthoc_low_lr_50_to_60":
        ...
        if args.run_stage == "posthoc_low_lr_50_to_60":
            ...
            if source_epoch == 50:
                ...
                if observed != expected:
                    raise RuntimeError(
                        "Invalid Global-direct epoch-50 source:\n"
                        + json.dumps(
        ...
        if args.run_stage == "pilot_extension_15_to_50":
            ...
            if int(checkpoint["epoch"]) == 15:
                ...
                if observed_source != expected_source:
                    raise RuntimeError(
                        "Invalid global-direct pilot continuation source:\n"
                        + json.dumps(
                            {"expected": expected_source, "observed": observed_source},
                            indent=2,
                        )
                    )
            elif source_config.get("run_stage") != "pilot_extension_15_to_50":
                raise RuntimeError("Post-epoch-15 resume is not an audited extension checkpoint")
            previous_extension_hash = source_config.get(
