...
from src.dataset_paired_multicoil_complementary import PairedMulticoilDataset
    ...
    def __init__(self, dataset, batch_size: int, shuffle: bool = False, seed: int = 42):
        ...
        for idx, record in enumerate(dataset.records):
            ...
            with h5py.File(pd_path, "r") as hf:
                ...
                # fastMRI multicoil kspace shape is usually:
                # [num_slices, num_coils, height, width]
                # Batch collation requires identical spatial dimensions.
...
def l1_metric(target: np.ndarray, pred: np.ndarray) -> float:
    # Match single evaluation and training-style target-scaled L1.
...
def evaluate_one_R(
    ...
    print("=" * 80)
    print(f"Evaluating joint VarNet R={R}")
    print("=" * 80)
    ...
    dataset = PairedMulticoilDataset(
        ...
        pd_mask_type=pd_mask_type,
        pdfs_mask_type=pdfs_mask_type,)
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        if batch_idx == 1 or batch_idx % 25 == 0:
            print(f"R={R} | batch {batch_idx}/{len(loader)}")
    ...
    per_slice_path = output_dir / f"joint_R{R}_val_per_slice_metrics.csv"
    patient_path = output_dir / f"joint_R{R}_val_patient_level_metrics.csv"
    summary_path = output_dir / f"joint_R{R}_val_summary.csv"
    summary_json_path = output_dir / f"joint_R{R}_val_summary.json"
...
def main():
    ...
    parser.add_argument("--pd_mask_type", type=str, default="equispaced", choices=["gaussian_vd", "gaussian", "equispaced"], help="Mask type for PD branch.")
    parser.add_argument("--pdfs_mask_type", type=str, default="gaussian_vd", choices=["gaussian_vd", "gaussian", "equispaced"], help="Mask type for PD-FS branch.")

    args = parser.parse_args()

    print("=" * 80)
    print("Evaluating complementary-mask revised joint")
    print(f"PD mask type: {args.pd_mask_type}")
    print(f"PD-FS mask type: {args.pdfs_mask_type}")
    print("Acceleration R: 4, 6, 8")
    print("=" * 80)
    ...
    for R, ckpt_path in checkpoints.items():
        summary = evaluate_one_R(
            ...
            pd_mask_type=args.pd_mask_type,
            pdfs_mask_type=args.pdfs_mask_type,
            device=device,
        )
