...
def curriculum_mixture(epoch: int, curriculum: str) -> Dict[str, float]:
    ...
    ``paired3`` is the V2.1 three-epoch paired discrimination curriculum. Every
    sampled corruption is accompanied by its clean counterpart in the
    training script, so this mixture deliberately contains no clean samples.
    ``fusioncal3`` reuses the validated paired3 epoch-3 mixture for three
    epochs of fusion-only calibration.
    ``smoke5`` is the legacy compressed five-epoch screening curriculum.
    ``formal15`` is the staged curriculum for a fresh 15-epoch pilot.
    """
    if epoch < 1:
        raise ValueError("Epoch numbering starts at one.")

    if curriculum == "paired3":
        schedules = {
            1: {"border": 0.50, "shift_mild": 0.50},
            2: {
                "border": 0.15,
                "shift": 0.30,
                "wrong_slice": 0.25,
        ...
        mixture = schedules.get(epoch, schedules[3])
    elif curriculum == "fusioncal3":
        # Fusion-only calibration. All three epochs reuse the audited Stage-A
        # epoch-3 distribution. Blur and noise are intentionally excluded
        # because they are not part of the frozen final audit protocol.
        mixture = {
            "border": 0.15,
            "shift": 0.30,
            "wrong_slice": 0.25,
...
def ranking_margin_by_scale(
    corrupted_target: torch.Tensor,
    record: Mapping[str, Any],
) -> torch.Tensor:
    """Return a severity-aware margin consistent with absolute supervision.

    The result has the same four-scale shape as ``corrupted_target`` and is
    always capped by ``1 - target``.  Consequently, the paired ranking term
    can never demand a separation that contradicts BCE supervision.
    """
    if corrupted_target.ndim != 1 or corrupted_target.numel() != len(SCALE_NAMES):
        raise ValueError(
            "corrupted_target must contain one value per scale; got "
            f"{tuple(corrupted_target.shape)}."
        )
    condition = str(record.get("condition", ""))
    target_gap = (1.0 - corrupted_target).clamp_min(0.0)
    ...
    if condition == "shift":
        ...
        delta_z = float(record.get("delta_z_norm", 0.0))
        # The registered range maps delta_z=0.05 to the minimum margin and
        # delta_z=0.20 to the maximum margin.
        severity = min(1.0, max(0.0, (delta_z - 0.05) / 0.15))
        maximum_margin = 0.025 + 0.075 * severity
    elif condition == "wrong_patient":
        maximum_margin = 0.20
    elif condition == "missing":
        maximum_margin = 0.20
    elif condition in {"blur", "noise"}:
        maximum_margin = 0.10
    elif condition in {"border", "clean"}:
        # Border and clean controls use target-aligned consistency in the
        # training loss rather than a one-sided ranking margin.
        return target_gap
    else:
        raise ValueError(f"Unsupported ranking condition {condition!r}.")

    cap = torch.full_like(target_gap, float(maximum_margin))
    return torch.minimum(target_gap, cap)
...
def paired_discrimination_loss(
    ...
    """Matched target-aligned consistency/ranking loss used by Stage A."""
    if q_clean.shape != q_corrupted.shape or q_clean.ndim != 3:
        raise ValueError(
            "Expected matched q tensors [B,cascades,scales], got "
            f"{tuple(q_clean.shape)} and {tuple(q_corrupted.shape)}."
        )
    if corrupted_targets.shape != (q_clean.shape[0], q_clean.shape[-1]):
        raise ValueError(
            "Expected corrupted_targets [B,scales], got "
            f"{tuple(corrupted_targets.shape)}."
        )
    if len(records) != q_clean.shape[0]:
        raise ValueError("Pair records do not match q batch size.")

    losses: List[torch.Tensor] = []
    requested_margins: List[float] = []
    realised_gaps: List[float] = []
    consistency_residuals: List[float] = []
    violation_rates: List[float] = []

    for sample_index, record in enumerate(records):
        condition = str(record.get("condition", ""))
        gap = q_clean[sample_index] - q_corrupted[sample_index]
        target_or_margin = ranking_margin_by_scale(
            corrupted_targets[sample_index], record
        )

        if condition in {"border", "clean"}:
            residual = gap - target_or_margin[None, :]
            sample_loss = F.relu(
                residual.abs() - float(consistency_tolerance)
            ).mean()
            consistency_residuals.append(
                float(residual.detach().abs().mean().item())
            )
            requested_margin = float(target_or_margin.mean().item())
        else:
            sample_loss = F.relu(target_or_margin[None, :] - gap).mean()
            if condition == "missing":
                sample_loss = 0.5 * sample_loss
            requested_margin = float(target_or_margin.mean().item())
            violation_rates.append(
                float(
                    (gap.detach() < target_or_margin[None, :])
                    .float()
                    .mean()
                    .item()
                )
        ...
        requested_margins.append(requested_margin)
        realised_gaps.append(float(gap.detach().mean().item()))

    def mean_or_nan(values: Sequence[float]) -> float:
        return float(sum(values) / len(values)) if values else float("nan")

    return torch.stack(losses).mean(), {
        "q_clean": float(q_clean.detach().mean().item()),
        "q_corrupted": float(q_corrupted.detach().mean().item()),
        "q_gap": float((q_clean - q_corrupted).detach().mean().item()),
        "requested_margin": mean_or_nan(requested_margins),
        "border_target_residual": mean_or_nan(consistency_residuals),
        "rank_violation_rate": mean_or_nan(violation_rates),
    }
