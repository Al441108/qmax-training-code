...
class ReliabilityHead(nn.Module):
    """Per-scale reliability score using detached cross-contrast evidence."""
    ...
    def forward(self, evidence: torch.Tensor, roi_margin: int) -> torch.Tensor:
        logits = self.layers(evidence)
        logits = _central_roi(logits, roi_margin)
        return torch.sigmoid(logits.mean(dim=(-2, -1)))[:, 0]
...
class M2GDv2FeatureFusion(nn.Module):
    ...
    def __init__(
        ...
        evidence_chans = 4 * target_chans + 1
        hidden_chans = max(8, target_chans // 2)
    ...
    def forward(
        ...
        reliability_evidence = _cross_contrast_evidence(
            target_feature.detach(), adapted.detach()
        )
        q_hat = self.reliability_head(
        ...
        gate_evidence = _cross_contrast_evidence(target_feature, adapted)
...
class M2GDv2Regulariser(nn.Module):
...
class M2GDv2VarNetBlock(nn.Module):
    def __init__(
        self,
        chans: int = 18,
        pools: int = 4,
        ...
        super().__init__()
        self.regulariser = M2GDv2Regulariser(
            chans=chans,
            pools=pools,
...
class M2GDv2AuxPDVarNet(nn.Module):
    """M2-U-initialised, per-scale cross-contrast reliability VarNet."""
