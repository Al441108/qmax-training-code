...
class JointUNet4ch(nn.Module):
    ...
    def __init__(
        ...
        # Important for VarNet stability:
        # initialise learned regulariser update near zero so the first cascades
        # start close to data-consistent reconstruction instead of injecting a
        # large random k-space model term.
    ...
    def forward(self, x):
        ...
        # Per-instance normalisation, similar in spirit to fastMRI NormUnet,
        # but this regulariser predicts an update/model term, so we later
        # rescale by std only rather than adding the input mean back.
        b = x.shape[0]
        ...
        # Rescale update to input magnitude. Do not add mean, because zero
        # output should remain a neutral zero regularisation update.
        out = out * std
