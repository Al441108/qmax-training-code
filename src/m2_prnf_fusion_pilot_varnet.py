...
"""Fusion-operator pilot built on the frozen M2-PRNF v1.3 backbone.
...
class ComplementaryResidualHead(nn.Module):
    ...
    is fixed to one.  This gives a neutral initial correction without the
...
class FusionPilotScaleController(nn.Module):
    """Scale controller supporting the four pre-registered fusion designs."""
    ...
    def __init__(
        ...
        if fusion_design in {"residual_only", "hybrid_direct_residual"}:
            with torch.random.fork_rng(devices=[]):
    ...
    def forward(
        ...
        if self.complement is not None:
            # The four-scale controller is called in every cascade.  Saving all
            # complementary activations exceeds the 95-GiB allocation visible
            # on the target GH200 at anatomical batch 4.  Non-reentrant
            # checkpointing recomputes this small head during backward while
            # preserving the frozen batch-4 optimisation protocol.
        ...
        if self.fusion_design == "legacy_local_direct":
            ...
            direct_term = zeros
            residual_term = m * q * need_factor * residual_raw
...
class M2PRNFFusionPilotVarNet(M2PRNFAuxPDVarNet):
    """v1.3 reconstruction backbone with a selectable fusion operator."""
